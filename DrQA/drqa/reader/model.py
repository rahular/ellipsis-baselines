...
class DocReader(object):
    ...
    def expand_dictionary(self, words):
        ...
        if len(to_add) > 0:
            ...
            self.network.embedding = torch.nn.Embedding(
                self.args.vocab_size, self.args.embedding_dim, padding_idx=0
            )
    ...
    def load_embeddings(self, words, embedding_file):
        ...
        logger.info(
            "Loading pre-trained embeddings for %d words from %s"
    ...
    def init_optimizer(self, state_dict=None):
        ...
        if self.args.optimizer == "sgd":
            ...
            self.optimizer = optim.Adamax(
                parameters, weight_decay=self.args.weight_decay
            )
        else:
