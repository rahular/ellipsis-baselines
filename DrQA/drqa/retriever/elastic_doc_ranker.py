...
class ElasticDocRanker(object):
    ...
    def get_doc_index(self, doc_id):
        ...
        result = self.es.search(
            index=self.elastic_index, body={"query": {"match": {field_index: doc_id}}}
        )
    ...
    def get_doc_id(self, doc_index):
        ...
        result = self.es.search(
            index=self.elastic_index, body={"query": {"match": {"_id": doc_index}}}
        )
    ...
    def closest_docs(self, query, k=1):
        ...
        doc_ids = [
            utils.get_field(row["_source"], self.elastic_field_doc_name) for row in hits
        ]
    ...
    def get_doc_ids(self):
        ...
        results = self.es.search(
            index=self.elastic_index, body={"query": {"match_all": {}}}
        )
        doc_ids = [
    ...
    def get_doc_text(self, doc_id):
        ...
        return (
            result if result is None else result["_source"][self.elastic_field_content]
        )
