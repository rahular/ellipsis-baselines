...
class DrQA(object):
    ...
    def process_batch(
        self, queries, candidates=None, top_n=1, n_docs=5, return_context=False
    ):
        ...
        for qidx in range(len(queries)):
            for rel_didx, did in enumerate(all_docids[qidx]):
                ...
                for sidx in range(start, end):
                    if (
                        len(q_tokens[qidx].words()) > 0
                        and len(s_tokens[sidx].words()) > 0
                    ):
                        examples.append(
                            {
        ...
        logger.info(
            "Processed %d queries in %.4f (s)" % (len(queries), time.time() - t0)
        )
