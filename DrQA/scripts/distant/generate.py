    ...
    for ans in answers:
        ...
        for idx in range(len(p_words)):
            if p_words[idx : idx + len(a_words)] == a_words:
                ...
                w_grams = Counter(
                    w_tokens.ngrams(n=2, uncased=True, filter_fn=utils.filter_ngram)
                )
...
def process(questions, answers, outfile, opts):
    ...
    workers = Pool(
        opts["workers"],
        initializer=init,
        initargs=(opts["tokenizer_class"], {}, opts["db_class"], {}),
    )
    ...
    with open(outfile + ".dstrain", "w") as f_train, open(
        outfile + ".dsdev", "w"
    ) as f_dev:
...
if __name__ == "__main__":
    ...
    dataset.add_argument(
        "--regex", action="store_true", help="Flag if answers are expressed as regexps"
    )
    dataset.add_argument(
        "--dev-split", type=float, default=0, help="Hold out for ds dev set (0.X)"
    )
    ...
    search.add_argument(
        "--char-max", type=int, default=1500, help="Maximum allowed context length"
    )
    search.add_argument(
        "--char-min", type=int, default=25, help="Minimum allowed context length"
    )
    search.add_argument(
        "--window-sz",
        type=int,
    ...
    general.add_argument(
        "--max-ex", type=int, default=5, help="Maximum matches generated per question"
    )
    general.add_argument(
        "--n-docs", type=int, default=5, help="Number of docs retrieved per question"
    )
    ...
    # Form options
    search_keys = ("regex", "match_threshold", "char_max", "char_min", "window_sz")
    opts = {
        "ranker_class": retriever.get_class(args.ranker),
        "tokenizer_class": tokenizers.get_class(args.tokenizer),
        "db_class": retriever.get_class(args.db),
