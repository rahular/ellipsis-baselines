...
def add_train_args(parser):
    ...
    runtime.add_argument("--gpu", type=int, default=-1, help="Run on a specific GPU")
    runtime.add_argument(
        "--data-workers",
        type=int,
        default=5,
    ...
    runtime.add_argument(
        "--num-epochs", type=int, default=40, help="Train data iterations"
    )
    runtime.add_argument(
...
def train(args, data_loader, model, global_stats):
    ...
    logger.info(
        "train: Epoch %d done. Time for epoch = %.2f (s)"
...
def validate_unofficial(args, data_loader, model, global_stats, mode):
    ...
    logger.info(
        "%s valid unofficial: Epoch = %d | start = %.2f | "
...
def validate_official(args, data_loader, model, global_stats, offsets, texts, answers):
    ...
    logger.info(
        "dev valid official: Epoch = %d | EM = %.2f | "
...
def eval_accuracies(pred_s, target_s, pred_e, target_e):
    ...
    for i in range(batch_size):
        ...
        if any(
            [
                1
                for _s, _e in zip(target_s[i], target_e[i])
...
def main(args):
    ...
    if args.sort_by_len:
        dev_sampler = data.SortedBatchSampler(
            dev_dataset.lengths(), args.test_batch_size, shuffle=False
        )
    else:
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if args.official_eval:
            result = validate_official(
                args, dev_loader, model, stats, dev_offsets, dev_texts, dev_answers
            )
        else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "DrQA Document Reader", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
