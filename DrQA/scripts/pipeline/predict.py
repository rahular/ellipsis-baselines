...
parser.add_argument(
    ...
    default="/tmp",
    help=(
        "Directory to write prediction file to " "(<dataset>-<model>-pipeline.preds)"
    ),
)
parser.add_argument(
    "--reader-model",
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help=("Expand dictionary to use all pretrained " "embeddings in this file"),
)
parser.add_argument(
    "--candidate-file",
    type=str,
...
parser.add_argument(
    "--n-docs", type=int, default=5, help="Number of docs to retrieve per query"
)
parser.add_argument(
    "--top-n", type=int, default=1, help="Number of predictions to make per query"
)
parser.add_argument(
    "--tokenizer",
    type=str,
...
parser.add_argument(
    "--parallel", action="store_true", help="Use data parallel (split across gpus)"
)
parser.add_argument(
    "--num-workers",
    type=int,
...
parser.add_argument(
    "--batch-size", type=int, default=128, help="Document paragraph batching size"
)
parser.add_argument(
    "--predict-batch-size", type=int, default=1000, help="Question batching size"
)
...
DrQA = pipeline.DrQA(
    ...
    ranker_config={"options": {"tfidf_path": args.retriever_model, "strict": False}},
...
with open(outfile, "w") as f:
    batches = [
        queries[i : i + args.predict_batch_size]
        for i in range(0, len(queries), args.predict_batch_size)
    ]
    for i, batch in enumerate(batches):
        logger.info("-" * 25 + " Batch %d/%d " % (i + 1, len(batches)) + "-" * 25)
        predictions = DrQA.process_batch(batch, n_docs=args.n_docs, top_n=args.top_n,)
