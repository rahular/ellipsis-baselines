...
def set_gpus(*gpus):
    ...
    print(
        "Setting CUDA_VISIBLE_DEVICES to: {}".format(os.environ["CUDA_VISIBLE_DEVICES"])
    )
...
def projection(inputs, output_size, initializer=None):
    return ffnn(
        inputs, 0, -1, output_size, dropout=None, output_weights_initializer=initializer
    )
...
def ffnn(
    ...
    if len(inputs.get_shape()) > 3:
        raise ValueError(
            "FFNN with rank {} not supported".format(len(inputs.get_shape()))
        )
    ...
    for i in range(num_hidden_layers):
        hidden_weights = tf.get_variable(
            "hidden_weights_{}".format(i), [shape(current_inputs, 1), hidden_size]
        )
        hidden_bias = tf.get_variable("hidden_bias_{}".format(i), [hidden_size])
        current_outputs = tf.nn.relu(
    ...
    output_weights = tf.get_variable(
        "output_weights",
        [shape(current_inputs, 1), output_size],
...
def cnn(inputs, filter_sizes, num_filters):
    ...
    for i, filter_size in enumerate(filter_sizes):
        ...
        conv = tf.nn.conv1d(
            inputs, w, stride=1, padding="VALID"
    ...
    def _block_orthonormal_initializer(self, output_sizes):
        def _initializer(shape, dtype=np.float32, partition_info=None):
            ...
            params = np.concatenate(
                [
                    initializer([shape[0], o], dtype, partition_info)
